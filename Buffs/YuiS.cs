...
namespace AlchemistNPC.Buffs
{
	public class YuiS : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			player.GetModPlayer<AlchemistNPCPlayer>().YuiS = true;
			player.GetModPlayer<AlchemistNPCPlayer>().Yui = false;
			...
			{
				Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ModContent.ProjectileType<Projectiles.Pets.YuiS>(), 0, 0f, player.whoAmI, 0f, 0f);
			}
