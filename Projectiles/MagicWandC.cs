...
namespace AlchemistNPC.Projectiles
{
	public class MagicWandC : ModProjectile
	{
		...
		public void DrawLaser(Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			{
				texture = Mod.Assets.Request<Texture2D>("Projectiles/MagicWandC").Value;
			...
			{
				texture = Mod.Assets.Request<Texture2D>("Projectiles/MagicWandB").Value;
				#region Draw laser body
				for (float i = transDist; i <= Distance; i += step)
				{
					Color c = Color.White;
			...
			if (Charge == MAX_CHARGE && supercharge >= 180 && hypercharge >= 180)
			{
				texture = Mod.Assets.Request<Texture2D>("Projectiles/MagicWandA").Value;
				#region Draw laser body
				for (float i = transDist; i <= Distance; i += step)
				{
					Color c = Color.White;
		...
		public override void AI()
		{
			...
			{
				Projectile.Kill();
				((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).chargetime = 0;
			}
			else
			{
