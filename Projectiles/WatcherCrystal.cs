			...
			for (int i = 0; i < 200; i++)
            {
                ...
                if (distance < 520f && target.catchItem == 0 && !target.friendly && target.active && target.type != 488)
                {
                    if (Projectile.ai[0] > 60f) // Time in (60 = 1 second) 
                    {
                        ...
                        shootToX *= distance * 3;
                        shootToY *= distance * 3;
                        Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, shootToX, shootToY, 617, Projectile.damage, 0, Main.myPlayer, 0f, 0f);
                        Projectile.ai[0] = 0f;
                    }
                }
            }
