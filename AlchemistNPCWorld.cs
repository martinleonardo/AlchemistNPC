...
{
    public class AlchemistNPCWorld : ModSystem
    {
        ...
        public override void PostUpdateWorld()
        {
            ...
            {
				int playerNo;
                for (playerNo = 0; playerNo < 40; playerNo++) {
                    if (Main.player[playerNo].active) {
                        Player player = Main.player[playerNo];
                        ...
                        int index2 = Projectile.NewProjectile(player.GetProjectileSource_TileInteraction((int)X, (int)Y), X, Y, SpeedX * 1.5f, SpeedY * Main.rand.NextFloat(0.8f, 1.2f), ProjectileType<Projectiles.SymbioteMeteor>(), 10000, 8f, player.whoAmI, 0f, 0f);
                ...
                if (playerNo == 255) {
				    throw new UsageException("Could not find player");
			    }
            }
        }
