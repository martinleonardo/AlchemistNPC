		...
		if (counter == 30)
			{
				if (player.direction == 1)
				{
					...
					vel *= 0f;
					Projectile.NewProjectile(player.GetProjectileSource_Item(Item), player.Center.X+20+Main.rand.Next(15), player.Center.Y-50-Main.rand.Next(30), vel.X, vel.Y, ProjectileType<Projectiles.SharpBone>(), Item.damage*2, 0, player.whoAmI);
					Projectile.NewProjectile(player.GetProjectileSource_Item(Item), player.Center.X+20+Main.rand.Next(15), player.Center.Y-40-Main.rand.Next(30), vel.X, vel.Y, ProjectileType<Projectiles.SharpBone>(), Item.damage*2, 0, player.whoAmI);
					counter = 0;
					}
				}
				if (player.direction == -1)
