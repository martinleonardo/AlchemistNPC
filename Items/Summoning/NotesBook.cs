...
namespace AlchemistNPC.Items.Summoning
{
	public class NotesBook : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.makeNPC = (short)NPCType<NPCs.OtherworldlyPortal>();
		}
		...
		public override bool CanUseItem(Player player)
		{
			if (Main.myPlayer == player.whoAmI)
			{
				...
				if (!NPC.AnyNPCs(NPCType<NPCs.OtherworldlyPortal>()) && !NPC.AnyNPCs(NPCType<NPCs.Explorer>()) && Main.eclipse && !Collision.SolidCollision(vector2, player.width, player.height))
				{
					return true;
				}
			}
