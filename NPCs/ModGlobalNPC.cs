    ...
    public class ModGlobalNPC : GlobalNPC
    {
        ...
        public override bool PreAI(NPC npc)
        {
            for (int k = 0; k < 255; k++)
            {
                ...
                if (player.active)
                {
                    if (player.HasBuff(ModContent.BuffType<Buffs.NULL>()))
                    {
                        return false;
                    }
                }
        ...
        {
            if (npc.townNPC && npc.HasBuff(ModContent.BuffType<Buffs.IField>()))
            {
                int respawn = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, npc.type);
                Main.npc[respawn].buffImmune[ModContent.BuffType<Buffs.IField>()] = false;
                Main.npc[respawn].AddBuff(ModContent.BuffType<Buffs.IField>(), 3600);
                return true;
        ...
        {
            if (item.type == ModContent.ItemType<Items.Weapons.EdgeOfChaos>())
            {
                S++;
            }
            if (item.type == ModContent.ItemType<Items.Weapons.HolyAvenger>())
            {
                C++;
            }
            if (item.type == ModContent.ItemType<Items.Weapons.LightOfCeraSumat>())
            {
                C += 3;
            }
            if (npc.HasBuff(ModContent.BuffType<Buffs.CurseOfLight>()))
            {
                damage += damage / 5;
            }
            if (npc.HasBuff(ModContent.BuffType<Buffs.SymbolOfPain>()))
            {
                damage += damage / 4;
            }
            if (player.HasBuff(ModContent.BuffType<Buffs.ExecutionersEyes>()))
            {
                damage += (damage / 20) * 3;
            }
        }
        ...
        public override void ModifyHitByProjectile(NPC npc, Projectile projectile, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            if (player.HasBuff(ModContent.BuffType<Buffs.LaserBattery>()) && projectile.type == ProjectileID.UFOLaser)
            {
        ...
        {
            if (npc.HasBuff(ModContent.BuffType<Buffs.CurseOfLight>()) && Main.rand.Next(4) == 0)
            {
                damage /= 2;
            }
            if (npc.HasBuff(ModContent.BuffType<Buffs.SymbolOfPain>()))
        ...
        public override void GetChat(NPC npc, ref string chat)
        {
            ...
            int Angela = NPC.FindFirstNPC(NPCType<NPCs.Operator>());
        ...
        public override void SetupShop(int type, Chest shop, ref int nextSlot)
        {
            for (int k = 0; k < 255; k++)
            {
                ...
                if (player.active)
                {
                    if (type == NPCType<NPCs.Brewer>() || type == NPCType<NPCs.Alchemist>() || type == NPCType<NPCs.YoungBrewer>())
                    {
                        for (nextSlot = 0; nextSlot < 40; ++nextSlot)
                        {
							...
							*/
                            if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier4)
                            {
                                shop.item[nextSlot].shopCustomPrice -= shop.item[nextSlot].shopCustomPrice / 2;
                            }
                            else if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier3)
                            {
                                shop.item[nextSlot].shopCustomPrice -= ((shop.item[nextSlot].shopCustomPrice / 20) * 7);
                            }
                            else if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier2)
                            {
                                shop.item[nextSlot].shopCustomPrice -= shop.item[nextSlot].shopCustomPrice / 4;
                            }
                            else if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier1)
                            {
        ...
        public override void SetDefaults(NPC npc)
        {
            ...
            if (npc.type == NPCType<NPCs.Alchemist>() || npc.type == NPCType<NPCs.Brewer>() || npc.type == NPCType<NPCs.YoungBrewer>() || npc.type == NPCType<NPCs.Jeweler>() || npc.type == NPCType<NPCs.Architect>() || npc.type == NPCType<NPCs.Musician>() || npc.type == NPCType<NPCs.Tinkerer>())
            {
                if (NPC.downedMoonlord)
                {
                    npc.lifeMax = 500;
        ...
        {
            if (npc.HasBuff(ModContent.BuffType<Buffs.ChaosState>()))
            {
            ...
            if (corrosion)
            {
                ...
                {
                    int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, DustType<Dusts.RainbowDust>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 3.5f);
            ...
            if (npc.HasBuff(ModContent.BuffType<Buffs.ArmorDestruction>()))
            {
                if (Main.rand.Next(4) < 2)
                {
                    int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, DustType<Dusts.BastScroll>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 3.5f);
                    Main.dust[dust].noGravity = true;
                    Main.dust[dust].velocity *= 1.8f;
                    Main.dust[dust].velocity.Y -= 0.5f;
            ...
            if (justitiapale)
            {
                if (Main.rand.Next(4) < 2)
                {
                    int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, DustType<Dusts.JustitiaPale>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 3.5f);
                    Main.dust[dust].noGravity = true;
                    Main.dust[dust].velocity *= 1.8f;
                    Main.dust[dust].velocity.Y -= 0.5f;
                    if (Main.rand.Next(3) == 0)
        ...
        public override bool PreKill(NPC npc)
        {
            ...
            if (AlchemistNPCWorld.foundAntiBuffMode)
            {
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.KingSlimeBooster>());
                }
                if (npc.type == NPCID.EyeofCthulhu)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.EyeOfCthulhuBooster>());
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.BrokenBooster1>());
                }
                if (npc.type == NPCID.EaterofWorldsHead && !NPC.AnyNPCs(NPCID.EaterofWorldsTail))
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.EaterOfWorldsBooster>());
                }
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.BrainOfCthulhuBooster>());
                }
                if (npc.type == NPCID.QueenBee)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.QueenBeeBooster>());
                }
                if (npc.type == NPCID.SkeletronHead)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.SkeletronBooster>());
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.BrokenBooster2>());
                }
                if (npc.type == NPCID.WallofFlesh)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.WoFBooster>());
                }
                if (npc.type == NPCID.SkeletronPrime)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.PrimeBooster>());
                }
                if (npc.type == NPCID.Spazmatism && !NPC.AnyNPCs(NPCID.Retinazer))
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.TwinsBooster>());
                }
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.DestroyerBooster>());
                }
                if (npc.type == NPCID.Plantera)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.PlanteraBooster>());
                }
                if (npc.type == NPCID.Golem)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.GolemBooster>());
                }
                if (npc.type == NPCID.DukeFishron)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.FishronBooster>());
                }
                if (npc.type == NPCID.CultistBoss)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.CultistBooster>());
                }
                if (npc.type == NPCID.MoonLordCore)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.MoonLordBooster>());
                }
                if (npc.type == NPCID.DD2DarkMageT1 || npc.type == NPCID.DD2DarkMageT3)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.DarkMageBooster>());
                }
                if (npc.type == NPCID.DD2OgreT2 || npc.type == NPCID.DD2OgreT3)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.OgreBooster>());
                }
                if (npc.type == NPCID.DD2Betsy)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.BetsyBooster>());
                }
                if (npc.type == NPCID.GoblinSummoner)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.GSummonerBooster>());
                }
                if (npc.type == NPCID.IceGolem)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.IceGolemBooster>());
                }
                if (npc.type == NPCID.PigronCorruption || npc.type == NPCID.PigronHallow || npc.type == NPCID.PigronCrimson)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.PigronBooster>());
                }
                if (npc.type == NPCID.MartianSaucerCore)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Boosters.MartianSaucerBooster>());
                }
                for (int k = 0; k < 255; k++)
                {
                    Player player = Main.player[k];
            ...
            for (int k = 0; k < 255; k++)
            {
                ...
                if (player.active)
                {
                    ...
                    {
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Summoning.KnucklesUgandaDoll>());
                    }
                    if (Main.expertMode && AlchemistNPC.modConfiguration.CoinsDrop)
                    {
                        ...
                        if (ModLoader.GetMod("SpiritMod") != null)
                        {
                            if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("Infernon")))
                            {
                                ...
                                else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier 2 Reversity coins and now has " + modPlayer.RCT2 + " in total."), new Color(255, 255, 255));
                            }
                        }
						*/
                        if (npc.type == NPCID.DD2DarkMageT1 || npc.type == NPCID.DD2DarkMageT3)
                        {
                            ...
                            else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier 1 Reversity coins and now has " + modPlayer.RCT1 + " in total."), new Color(255, 255, 255));
                        }
                        if (npc.type == NPCID.DD2OgreT2 || npc.type == NPCID.DD2OgreT3)
                        {
                            ...
                            else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier 3 Reversity coins and now has " + modPlayer.RCT3 + " in total."), new Color(255, 255, 255));
                        }
                        if (npc.type == NPCID.DD2Betsy)
                        {
                            int number = Main.rand.Next(3, 6);
                            ...
                            else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier 4 Reversity coins and now has " + modPlayer.RCT4 + " in total."), new Color(255, 255, 255));
                        }
                        if (npc.type == NPCID.MoonLordCore)
                        {
                            int number = Main.rand.Next(12, 15);
            ...
            if (npc.type == NPCID.MoonLordCore)
            {
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Notes.TornNote9>());
                }
                if (AlchemistNPC.modConfiguration.TinkererSpawn)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 3);
                }
            }
            ...
            if (!npc.SpawnedFromStatue)
            {
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.HolyAvenger>(), 1, false, 81);
                }
                if (Main.rand.Next(25000) == 0)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.Penetrator>(), 1, false, 82);
                }
                if (Main.rand.Next(25000) == 0)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.TomeOfOrder>(), 1, false, 83);
                }
                if (Main.rand.Next(25000) == 0)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.FlaskoftheAlchemist>(), 1, false, 82);
                }
                if (Main.rand.Next(25000) == 0)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.CounterMatter>(), 1, false);
                }
                if (Main.rand.Next(33333) == 0)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Equippable.CrackedCrown>(), 1, false);
                }
            }
                ...
                if (player.active)
                {
                    ...
                    if (Main.expertMode && AlchemistNPC.modConfiguration.CoinsDrop)
                    {
                        ...
                        switch (npc.type)
                        {
                            ...
                            case (NPCID.QueenSlimeBoss): number = Main.rand.Next(1, 3); tier = 3; break;
                            ...
                            case (NPCID.HallowBoss): number = Main.rand.Next(6, 9); tier = 4; break;
                        ...
                        if (number > 0)
                        {
                            switch (tier)
                            {
                                case 1:
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT1 + " in total."), new Color(255, 255, 255));
                                    break;
                                case 2:
                                    modPlayer.RCT2 += number;
                                    if (Main.netMode == 2) SyncPlayerVariables(player);
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT2 + " in total."), new Color(255, 255, 255));
                                    break;
                                case 3:
                                    modPlayer.RCT3 += number;
                                    if (Main.netMode == 2) SyncPlayerVariables(player);
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT3 + " in total."), new Color(255, 255, 255));
                                    break;
                                case 4:
                                    modPlayer.RCT4 += number;
                                    if (Main.netMode == 2) SyncPlayerVariables(player);
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT4 + " in total."), new Color(255, 255, 255));
                                    break;
                                case 5:
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT5 + " in total."), new Color(255, 255, 255));
                                    break;
                                case 6:
                                    ...
                                    else ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Player " + player.name + " got " + number + " tier " + tier + " Reversity coins and now has " + modPlayer.RCT6 + " in total."), new Color(255, 255, 255));
                                    break;
                            }
                        }
                    }
                    if (player.HasBuff(ModContent.BuffType<Buffs.Snatcher>()) && !npc.friendly && npc.type != 14 && npc.type != 135 && !npc.SpawnedFromStatue && npc.type != 1 && npc.type != 535)
                    {
                        // IMPLEMENT WHEN WEAKREFERENCES FIXED
                        /*
                        if (ModLoader.GetMod("Redemption") != null)
						...
						*/
                        // Delete next 3 lines when commented code is reimplemented
                        modPlayer.SnatcherCounter++;
                        if (Main.netMode == 2)
                            SyncPlayerVariables(player);
                    }
                    if (player.HeldItem.type == ModContent.ItemType<Items.Weapons.BloodthirstyBlade>() && !npc.SpawnedFromStatue)
                    {
                    ...
                    if (player.HeldItem.type == ModContent.ItemType<Items.Weapons.ChristmasW>() && Main.rand.NextBool(33))
                    {
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.Present);
                    }
                    if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).Extractor && npc.boss == true && npc.lifeMax >= 50000 && Main.rand.NextBool(3))
                    {
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Materials.SoulEssence>());
                    }
                    if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).Extractor && npc.boss == true && npc.lifeMax >= 55000 && Main.rand.NextBool(10))
                    {
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Materials.HateVial>());
                    }
                    if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).TimeTwist && npc.boss == false && Main.rand.NextBool(4))
                    {
                        npc.NPCLoot();
                        break;
                    }
        ...
        {
            if ((npc.type == NPCID.BloodCrawler || npc.type == NPCID.BloodCrawlerWall) || (npc.type == NPCID.WallCreeper || npc.type == NPCID.WallCreeperWall))
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.SpiderFangarang>(), 10));
            }
            if ((npc.type == NPCID.JungleCreeper || npc.type == NPCID.JungleCreeperWall))
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.FangBallista>(), 20));
            }
            if ((npc.type == NPCID.WallCreeper || npc.type == NPCID.WallCreeperWall))
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.SwordofArachna>(), 20));
            }
            if (npc.lifeMax >= 25000 && npc.boss)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PerfectionToken>(), 20));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.Devilsknife>(), 200));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.WailOfBanshee>(), 33));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.ExecutionersEyes>(), 33));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.SymbolOfPain>(), 33));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.MeteorSwarm>(), 33));
            }
            if (npc.lifeMax >= 75000 && npc.boss && NPC.downedMoonlord)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.CloakOfFear>(), 33));
            }
            if (npc.type == NPCID.EyeofCthulhu)
            {
                if (!NPC.downedBoss1)
            ...
            if (npc.type == NPCID.WallofFlesh)
            {
                ...
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PHD>(), 1));
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Materials.BrokenDimensionalCasket>(), 1));
            }
            if (npc.type == NPCID.DungeonGuardian)
            {
                if (!Main.expertMode)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Materials.EmagledFragmentation>(), 1, 20, 30));
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Equippable.OtherworldlyAmulet>(), 10));
                }
                if (Main.expertMode)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Materials.EmagledFragmentation>(), 1, 40, 50));
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Equippable.OtherworldlyAmulet>(), 5));
                }
            }
            ...
            if (npc.type == NPCID.Plantera)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Weapons.Hive>(), 20));    //Give mythical modifier
                //Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Weapons.Hive>(), 1, false, 83);
            }
            if (npc.type == NPCID.Golem)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.Fuaran>(), 10));
            }
            if (npc.boss && AlchemistNPC.modConfiguration.TinkererSpawn)
            {
                if (npc.type == NPCID.KingSlime)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube>(), 1, 1, 1));
                }
                if (npc.type == NPCID.EyeofCthulhu)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube>(), 1, 2, 2));
                }
                if (npc.type == NPCID.BrainofCthulhu)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube>(), 1, 2, 2));
                ...
                if (npc.type == NPCID.QueenBee)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube>(), 1, 3, 3));
                }
                if (npc.type == NPCID.SkeletronHead)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube>(), 1, 3, 3));
                ...
                if (npc.type == NPCID.WallofFlesh)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube2>(), 1, 2, 2));
                }
                ...
                if (npc.type == NPCID.SkeletronPrime)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube2>(), 1, 3, 3));
                }
                if (npc.type == NPCID.Spazmatism && !NPC.AnyNPCs(NPCID.Retinazer))
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube2>(), 1, 3, 3));
                ...
                if (npc.type == NPCID.Plantera)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube3>(), 1, 1, 1));
                }
                if (npc.type == NPCID.HallowBoss)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube3>(), 1, 2, 2));
                }
                if (npc.type == NPCID.Golem)
                {
                    npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Misc.PaperTube3>(), 1, 2, 2));
                ...
                if (Calamity != null)
                {
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("DesertScourgeHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("CrabulonIdle"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("HiveMindP2"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("PerforatorHive"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("SlimeGodCore"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("Cryogen"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("BrimstoneElemental"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("AquaticScourgeHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("SoulSeeker"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>());
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("Leviathan"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("AstrumAureus"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                    if (npc.type == (ModLoader.GetMod("CalamityMod").NPCType("PlaguebringerGoliath"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                }
                if (ModLoader.GetMod("ThoriumMod") != null)
                {
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("TheGrandThunderBirdv2"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("QueenJelly"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("Viscount"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("GraniteEnergyStorm"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("TheBuriedWarrior"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("ThePrimeScouter"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("BoreanStriderPopped"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("FallenDeathBeholder2"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("LichHeadless"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("ThoriumMod").NPCType("AbyssionReleased"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                }
                if (ModLoader.GetMod("ElementsAwoken") != null)
                {
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("Wasteland"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("Infernace"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("ScourgeFighter"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("RegarothHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("TheCelestial"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>());
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("Permafrost"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>());
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("Obsidious"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>());
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("Aqueous"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("TheEye"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>());
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("AncientWyrmHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>());
                    if (npc.type == (ModLoader.GetMod("ElementsAwoken").NPCType("TheGuardianFly"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 3);
                }
                if (ModLoader.GetMod("Redemption") != null)
                {
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("KingChicken"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("Thorn"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("TheKeeper"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("XenomiteCrystalPhase2"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("InfectedEye"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("KSCharge"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("VlitchCleaver"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 4);
                    if (npc.type == (ModLoader.GetMod("Redemption").NPCType("VlitchWormHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                }
                if (ModLoader.GetMod("SacredTools") != null)
                {
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("DecreeRun"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("RalnekPhase3"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("Jensen"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("Araneas"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>());
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("Raynare"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("SacredTools").NPCType("Primordia2"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                }
                if (ModLoader.GetMod("SpiritMod") != null)
                {
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("Scarabeus"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("ReachBoss"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("AncientFlyer"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("SteamRaiderHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("Dusking"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("SpiritCore"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("IlluminantMaster"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("SpiritMod").NPCType("Atlas"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 2);
                }
                if (ModLoader.GetMod("Laugicality") != null)
                {
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("DuneSharkron"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("Hypothema"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("Ragnar"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("AnDio3"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("Slybertron"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("TheAnnihilator"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("SteamTrain"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("Laugicality").NPCType("Etheria"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 3);
                }
                if (ModLoader.GetMod("pinkymod") != null)
                {
                    if (npc.type == (ModLoader.GetMod("pinkymod").NPCType("DeserteerMelee"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("pinkymod").NPCType("MythrilSlime"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 2);
                    if (npc.type == (ModLoader.GetMod("pinkymod").NPCType("Valdaris"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube2>(), 3);
                    if (npc.type == (ModLoader.GetMod("pinkymod").NPCType("GatekeeperHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 3);
                }
                if (ModLoader.GetMod("AAMod") != null)
                {
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("MushroomMonarch"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("FeudalFungus"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("GripOfChaosRed"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("GripOfChaosBlue"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>());
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("TruffleToad"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("Broodmother"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("Hydra"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 2);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("SerpentHead"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("Djinn"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("Saggitarius"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube>(), 3);
                    if (npc.type == (ModLoader.GetMod("AAMod").NPCType("Rajah"))) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.PaperTube3>(), 3);
                }
				*/
            }
        }
		...
		*/
        ...
        {
            if (npc.type == NPCType<NPCs.Knuckles>())
            {
                Player player = Main.player[(int)Player.FindClosest(npc.position, npc.width, npc.height)];
        ...
        public override void AI(NPC npc)
        {
            ...
            if (npc.type == NPCType<NPCs.BillCipher>())
            {
                ...
                if (npc.life <= npc.lifeMax * 0.6f && !i1)
                {
                    ...
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.VoodooDoll>());
                    i1 = true;
                }
                if (npc.life <= npc.lifeMax * 0.4f && !i2)
                {
                    Main.NewText(Language.GetTextValue("Mods.AlchemistNPC.Common.BillCipherChat6"), 10, 255, 10);
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.ScreamingHead>());
                    i2 = true;
                }
                if (npc.life <= npc.lifeMax * 0.2f && !i3)
                {
                    Main.NewText(Language.GetTextValue("Mods.AlchemistNPC.Common.BillCipherChat6"), 10, 255, 10);
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Misc.CursedMirror>());
                    i3 = true;
                }
                ...
                if (npc.life <= npc.lifeMax / 2 && !phase2)
                {
                    ...
                    for (int index1 = 0; index1 < 30; ++index1)
                    {
                        ...
                        Projectile.NewProjectile(npc.GetProjectileSpawnSource(), X, Y, 0f, 0f, ProjectileType<Projectiles.Madness>(), 200, 0, Main.myPlayer);
                    }
                }
