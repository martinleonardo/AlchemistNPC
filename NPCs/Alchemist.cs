...
namespace AlchemistNPC.NPCs
{
    ...
    public class Alchemist : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "获得符咒");
            LocalizationLoader.AddTranslation(text);
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "不要让来自丛林的黑暗法师用魅惑的智慧欺骗你。我和");
            LocalizationLoader.AddTranslation(text);
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "曾经是他的学徒。当他的课程变得过于黑暗时我退出了，但是");
            LocalizationLoader.AddTranslation(text);
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "对神秘学炼金术的兴趣却在不断增长");
            LocalizationLoader.AddTranslation(text);
            text = LocalizationLoader.CreateTranslation(Mod, "BrewElixir");
        ...
        public override void SetChatButtons(ref string button, ref string button2)
        {
            ...
            if (player.active)
            {
                ...
                {
                    if (player.inventory[j].type == ModContent.ItemType<Items.Misc.LifeElixir>())
                    {
                        LE = true;
                    }
            ...
            int Alchemist = NPC.FindFirstNPC(NPCType<NPCs.Alchemist>());
            if (player.name == "Gregg" && Main.npc[Alchemist].GivenName == Gregg && NPC.downedMoonlord && !Tantrum)
            {
                button2 = "Secret";
            }

            if (((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier1 == false && ((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier2 == false && ((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier3 == false && ((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).AlchemistCharmTier4 == false)
            {
                button2 = GetCharm;
            }
        }
            ...
            else
            {
                ...
                if (PS && AB)
                {
                    if (Main.player[Main.myPlayer].HasItem(ItemID.PhilosophersStone))
                    {
                        ...
                        {
                            if (inventory[k].type == ModContent.ItemType<Items.Materials.AlchemicalBundle>())
                            {
                                inventory[k].stack--;
                            }
                        }
                    }
                    player.QuickSpawnItem(ModContent.ItemType<Items.Misc.LifeElixir>());
                }
        ...
        public override void SetupShop(Chest shop, ref int nextSlot)
        {
            if (baseShop)
            {
				...
				*/
                if (NPC.downedBoss2)
                {
                    shop.item[nextSlot].SetDefaults(ItemType<Items.BeachTeleporterPotion>());
                    shop.item[nextSlot].shopCustomPrice = 20000;
                    nextSlot++;
                    shop.item[nextSlot].SetDefaults(ItemType<Items.JungleTeleporterPotion>());
                    shop.item[nextSlot].shopCustomPrice = 50000;
                    nextSlot++;
                }
                if (NPC.downedBoss3)
                {
                    shop.item[nextSlot].SetDefaults(ItemType<Items.OceanTeleporterPotion>());
                    shop.item[nextSlot].shopCustomPrice = 20000;
                    nextSlot++;
                    shop.item[nextSlot].SetDefaults(ItemType<Items.DungeonTeleportationPotion>());
                    shop.item[nextSlot].shopCustomPrice = 25000;
                    nextSlot++;
                }
                if (Main.hardMode)
                {
                    shop.item[nextSlot].SetDefaults(ItemType<Items.UnderworldTeleportationPotion>());
                    shop.item[nextSlot].shopCustomPrice = 50000;
                    nextSlot++;
                    shop.item[nextSlot].SetDefaults(ItemType<Items.BeaconTeleportator>());
                    shop.item[nextSlot].shopCustomPrice = 25000;
                    nextSlot++;
                }
                if (NPC.downedPlantBoss)
                {
                    shop.item[nextSlot].SetDefaults(ItemType<Items.TempleTeleportationPotion>());
				...
				*/
                ...
                for (int k = 0; k < 255; k++)
                {
                    ...
                    if (player.active)
                    {
                        for (int j = 0; j < player.inventory.Length; j++)
                        {
                            if (player.inventory[j].type == ModContent.ItemType<Items.Equippable.OtherworldlyAmulet>())
                            {
                                shop.item[nextSlot].SetDefaults(ItemType<Items.Materials.EmagledFragmentation>());
                                shop.item[nextSlot].shopCustomPrice = 100000;
                                nextSlot++;
                                OA = true;
                            }
                            if (player.inventory[j].type == ModContent.ItemType<Items.Equippable.Autoinjector>() && !OA)
                            {
                                shop.item[nextSlot].SetDefaults(ItemType<Items.Materials.EmagledFragmentation>());
                                shop.item[nextSlot].shopCustomPrice = 100000;
                                nextSlot++;
