...
namespace AlchemistNPC.NPCs
{
    ...
    public class Brewer : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "有一个传说中的悠悠球球被称为萨斯卡德.");
            LocalizationLoader.AddTranslation(text);
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "我曾经远离泰拉大陆去了解更多有关炼金术的信息。 在我的旅行中，我遇到了一位名叫Azanor的“魔法科学家”。 他向我展示了一种叫做''thaumaturgy''的魔法奥秘。");
            LocalizationLoader.AddTranslation(text);
        }
        ...
        public override void SetChatButtons(ref string button, ref string button2)
        {
            ...
            if (NPC.FindBuffIndex(119) >= 0 && NPC.AnyNPCs(NPCType<NPCs.Alchemist>()) && !NPC.AnyNPCs(NPCType<NPCs.YoungBrewer>()))
            {
                button2 = "???";
            }
            else
        ...
        public override void OnChatButtonClicked(bool firstButton, ref bool shop)
        {
            ...
            {
                if (NPC.HasBuff(119) && NPC.AnyNPCs(NPCType<NPCs.Alchemist>()) && !NPC.AnyNPCs(NPCType<NPCs.YoungBrewer>()))
                {
                    for (int k = 0; k < 255; k++)
                    {
                        ...
                        if (player.active)
                        {
                            NPC.SpawnOnPlayer(k, NPCType<NPCs.YoungBrewer>());
                            return;
                        }
                    }
                }
		...
		*/
        ...
        public override void SetupShop(Chest shop, ref int nextSlot)
        {
            ...
            if (Shop == 2)
            {
                shop.item[nextSlot].SetDefaults(ItemType<Items.SunshinePotion>());
                shop.item[nextSlot].shopCustomPrice = 15000;
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemType<Items.GreaterDangersensePotion>());
                shop.item[nextSlot].shopCustomPrice = 25000;
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemType<Items.Dopamine>());
                shop.item[nextSlot].shopCustomPrice = 10000;
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemType<Items.NatureBlessingPotion>());
                shop.item[nextSlot].shopCustomPrice = 25000;
                nextSlot++;
                if (NPC.downedBoss3)
                {
                    shop.item[nextSlot].SetDefaults(ItemType<Items.BewitchingPotion>());
                    shop.item[nextSlot].shopCustomPrice = 10000;
                    nextSlot++;
                    shop.item[nextSlot].SetDefaults(ItemType<Items.FortitudePotion>());
                    shop.item[nextSlot].shopCustomPrice = 15000;
                    nextSlot++;
                    if (Main.hardMode)
                    {
                        shop.item[nextSlot].SetDefaults(ItemType<Items.InvincibilityPotion>());
                        shop.item[nextSlot].shopCustomPrice = 30000;
                        nextSlot++;
                        shop.item[nextSlot].SetDefaults(ItemType<Items.TitanSkinPotion>());
                        ...
                        if (ModLoader.GetMod("CalamityMod") != null)
                        {
                            ...
                            {
                                shop.item[nextSlot].SetDefaults (ItemType<Items.HeartAttackPotion>());
                        ...
                        */
                        ...
                        {
                            shop.item[nextSlot].SetDefaults(ItemType<Items.DiscordPotion>());
                            shop.item[nextSlot].shopCustomPrice = 200000;
                            nextSlot++;
                        }
                        if (NPC.downedMechBoss1 && NPC.downedMechBoss2 && NPC.downedMechBoss3)
                        {
                            shop.item[nextSlot].SetDefaults(ItemType<Items.BlurringPotion>());
                            shop.item[nextSlot].shopCustomPrice = 150000;
                            nextSlot++;
                            if (NPC.downedPlantBoss)
                            {
                                shop.item[nextSlot].SetDefaults(ItemType<Items.NinjaPotion>());
                                shop.item[nextSlot].shopCustomPrice = 75000;
                                nextSlot++;
                            }
                            if (NPC.downedGolemBoss)
                            {
                                shop.item[nextSlot].SetDefaults(ItemType<Items.TrapsPotion>());
                                shop.item[nextSlot].shopCustomPrice = 50000;
                                nextSlot++;
                            }
                        }
