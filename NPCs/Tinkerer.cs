...
namespace AlchemistNPC.NPCs
{
	...
	public class Tinkerer : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
            ...
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "回馈");
            LocalizationLoader.AddTranslation(text);
        ...
        public override void SetChatButtons(ref string button, ref string button2)
        {
			...
			if (player.active)
			{
				...
				{
					if (player.inventory[j].type == ModContent.ItemType<Items.Misc.PaperTube>())
					{
						TubePresent = true;
					}
					if (player.inventory[j].type == ModContent.ItemType<Items.Misc.PaperTube2>())
					{
						TubePresent2 = true;
					}
					if (player.inventory[j].type == ModContent.ItemType<Items.Misc.PaperTube3>())
					{
						TubePresent3 = true;
					}
				}
            ...
            if (firstButton)
            {
				...
				if (NPC.downedMoonlord && !AlchemistNPCWorld.foundMP7 && AlchemistNPCWorld.foundT1 && AlchemistNPCWorld.foundT2 && AlchemistNPCWorld.foundT3)
				{
					for (int k = 0; k < 255; k++)
					{
						...
						if (player.active)
						{
							player.QuickSpawnItem(ModContent.ItemType<Items.Weapons.MP7>());
						}
					}
				}
				else if (AlchemistNPCWorld.foundT1 || AlchemistNPCWorld.foundT2 || AlchemistNPCWorld.foundT3)
				{
					...
					for (int k = 0; k < inventory.Length; k++)
					{
						...
						{
							if (inventory[k].type == ModContent.ItemType<Items.Misc.PaperTube>())
							{
								TubePresent = false;
								TubePresent2 = false;
								TubePresent3 = false;
						...
						{
							if (inventory[k].type == ModContent.ItemType<Items.Misc.PaperTube2>())
							{
								TubePresent = false;
								TubePresent2 = false;
								TubePresent3 = false;
						...
						{
							if (inventory[k].type == ModContent.ItemType<Items.Misc.PaperTube3>())
							{
								TubePresent = false;
								TubePresent2 = false;
								TubePresent3 = false;
        ...
        public override void SetupShop(Chest shop, ref int nextSlot)
        {
			...
			if (Shop == 3){
				if (Main.hardMode)
				{
					shop.item[nextSlot].SetDefaults (ModContent.ItemType<Items.Misc.MenacingToken>());
					nextSlot++;
					shop.item[nextSlot].SetDefaults (ModContent.ItemType<Items.Misc.LuckyToken>());
					nextSlot++;
					shop.item[nextSlot].SetDefaults (ModContent.ItemType<Items.Misc.ViolentToken>());
					nextSlot++;
					shop.item[nextSlot].SetDefaults (ModContent.ItemType<Items.Misc.WardingToken>());
					nextSlot++;
				}
			}
		}
